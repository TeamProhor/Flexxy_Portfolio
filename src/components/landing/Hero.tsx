...
export const Hero = ({ dict }: Readonly<HeroProps>) => {
  ...
  useGSAP(
    () => {
      ...
      if (prefersReducedMotion) {
        gsap.set(
          ".hero-pill, .hero-headline, .hero-accent, .hero-sub, .hero-loader, .hero-video",
          {
            opacity: 1,
            y: 0,
  ...
  return (
    <section
      id="hero"
      ref={containerRef}
      className="relative flex flex-col items-center text-center mt-10 md:mt-12"
    >
      <div
        aria-hidden="true"
      ...
      <h1
        className="text-4xl md:text-[72px] leading-[1.05] tracking-tight font-medium text-foreground max-w-4xl mx-auto"
      ...
      >
        ...
        >
          <span
            className="hero-accent font-serif-italic text-rose-500 font-normal tracking-normal px-6 relative z-10 inline-block"
            style={{ opacity: 0 }}
          >
            {dict.title2}
          </span>
      ...
      <div
        className="hero-loader mt-10 flex justify-center w-full"
        style={{ opacity: 0 }}
      >
        <Loader dict={dict.loader} />
      </div>

      <div
        className="hero-video mt-16 md:mt-20 w-full max-w-5xl mx-auto"
        style={{ opacity: 0 }}
      >
